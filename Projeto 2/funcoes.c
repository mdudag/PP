...
double teste(FILE *file, func_matriz funcao, int NUM_REPETICOES, double *A, double *B, double *C, 
  ...
  // Se for a versao paralela
  if (strcmp(speedup, "")==0) {
    double n_threads_d = atof(num_threads); 
    double speedup_d = tempo_seq_medio / tempo_medio;
    double eficiencia_d = (speedup_d / n_threads_d) * 100.0; // Eficiência real, pode ser >100% 
    
    snprintf(speedup, 10, "%f", speedup_d);
    snprintf(eficiencia, 10, "%f", eficiencia_d);
  }
  ...
  // Se for a funcao sequencial, retorna no tempo medio
  if (strcmp(speedup, "1.00x")==0) {
    return tempo_medio;
  }
  return 0;
}
